...
int main(){
	...
	double level = 4;
	double width = 200*level;
	double height = 150*level;
	int ssaa = 64;
    char image[]= "OutputImage.ppm";
	ofstream fout(image);
	fout << "P3\n" << width << " " << height << "\n255\n";
	cout << "rendering..." << endl;
	vec3 look_from = vec3(0,1.0,4);
	vec3 look_to = vec3(0, 0, -1);
	float aperture = 1.2;
	camera cam(look_from, look_to, vec3(0, 1, 0), 30, width / height, aperture, (look_from - look_to).length());
	hitable* list[5];
	list[0] = new sphere(vec3(0, 0, -1), 0.5, new lambertian(vec3(0.78, 0.4, 0.43)));
	list[1] = new sphere(vec3(0, -100.5, -1), 100, new lambertian(vec3(0.8, 0.8, 0.0)));
	list[2] = new sphere(vec3(1, 0, -1), 0.5, new metal(vec3(0.5, 0.52, 0.63), 0.1));
	list[3] = new sphere(vec3(-1, 0, -1), 0.5, new dielectric(1.6));
	list[4] = new sphere(vec3(-1, 0, -1), -0.49, new dielectric(1.6));
	hitable_list world = hitable_list(list, 5);
